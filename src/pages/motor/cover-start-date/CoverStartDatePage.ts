// src/pages/motor/cover-start-date/CoverStartDatePage.ts
...
const PAGE_KEY = "motor.cover-start-date" as const;

export class CoverStartDatePage extends basePage {
  ...
  async getAQuote() {
  await this.clickAlias("getAQuote");
  }
  ...
  async nextMonth() {
  await this.clickAlias("nextMonth");
  }

  async pleaseSelectDate() {
  await this.clickAlias("pleaseSelectDate");
  }

  async selectDate() {
  await this.clickAlias("selectDate");
  }
