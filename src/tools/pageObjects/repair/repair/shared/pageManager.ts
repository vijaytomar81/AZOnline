// src/tools/pageObjects/repair/repair/shared/pageManager.ts
...
import {
    ...
    getPageManagerFile,
} from "@pageObjectCommon/pagePaths";
...
export function buildPageManagerEntries(
    ...
    return loadAllPageMaps(mapsDir).map((item) => {
        ...
        return {
            ...
            importPath: `@pageObjectsObjects/${product}/${group}/${name}/${artifact.className}`,
        };
    });
}
...
export function readActualPageManagerState(
    ...
    const imports = [...tsText.matchAll(/import\s+\{\s*[A-Za-z0-9_]+\s*\}\s+from\s+"([^"]+)";/g)]
        .map((m) => m[1] ?? "")
        .filter((x) => x.startsWith("@pageObjectsObjects/"))
