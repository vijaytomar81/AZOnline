// src/pageObjectTools/page-object-generator/generator/pageObject.ts
...
function parseElementTypes(elementsTs: string): ElementTypeMap {
    const body = elementsTs.match(/export const elements = \{([\s\S]*?)\}\s*as const;/m)?.[1] ?? "";
...
function buildAliasesRegion(aliasesTs: string, elementTypes: ElementTypeMap): string {
    const pairs = extractAliasPairsFromAliasesTs(aliasesTs).sort((a, b) => a.aliasKey.localeCompare(b.aliasKey));
    const lines: string[] = [`  // <scanner:aliases>`, `  // This region is auto-managed. Do not edit by hand.`, ``];

    for (const { aliasKey, elementKey } of pairs) {
        lines.push(...methodLinesFor(aliasKey, elementTypes.get(elementKey) ?? "button"));
        lines.push(``);
    }
...
export function syncAliasesIntoPageObject(args: {
    ...
    if (!fs.existsSync(pageTsPath) || !fs.existsSync(aliasesTsPath) || !fs.existsSync(elementsTsPath)) return;

    const pageTs = fs.readFileSync(pageTsPath, "utf8");
    const aliasesTs = fs.readFileSync(aliasesTsPath, "utf8");
    const elementTypes = parseElementTypes(fs.readFileSync(elementsTsPath, "utf8"));
    ...
    const updated = pageTs.slice(0, lineStartIdx) + region + pageTs.slice(end + endToken.length);
