// src/toolingLayer/pageObjects/repair/repair/shared/pageManager.ts
...
export function buildPageManagerContent(
    ...
    lines.push(``, `export class PageManager {`, `  readonly page: Page;`);
...
export function readActualPageManagerState(
    ...
    const imports = [...tsText.matchAll(/import\s+\{\s*[A-Za-z0-9_]+\s*\}\s+from\s+"([^"]+)";/g)]
        .map((match) => match[1] ?? "")
        .filter((value) => value.startsWith("@businessLayer/pageObjects/objects/"))
