...
function replaceGeneratedRhs(line: string, oldKey: string, newKey: string): string {
    ...
    const bracket = new RegExp(
        `\\baliasesGenerated\$begin:math:display$\(\?\:\"\$\{oldKey\}\"\|\'\$\{oldKey\}\'\)\\$end:math:display$`,
        "g"
    );

    return line
        .replace(dot, toPropertyAccess("aliasesGenerated", newKey))
        .replace(bracket, toPropertyAccess("aliasesGenerated", newKey));
}

function rewriteRenamedMappings(
    filePath: string,
    renameMap: Record<string, string>
): boolean {
    if (!fs.existsSync(filePath) || Object.keys(renameMap).length === 0) {
        return false;
    }
    ...
    const after = before
        .split("\n")
        .map((line) =>
            Object.entries(renameMap).reduce(
                (acc, [oldKey, newKey]) =>
                    replaceGeneratedRhs(acc, oldKey, newKey),
                line
            )
        )
        .join("\n");
...
function appendMissingAliases(
    ...
    if (!fs.existsSync(filePath) || newGeneratedKeys.length === 0) {
        return false;
    }
    ...
    const lines = missing.map(
        (key) => `  ${key}: ${toPropertyAccess("aliasesGenerated", key)},`
    );
...
export function syncAliasesHumanFile(params: {
    ...
    const renamedChanged = rewriteRenamedMappings(aliasesHumanPath, renameMap);
    const appendedChanged = appendMissingAliases(
        aliasesHumanPath,
        newGeneratedKeys,
        log
    );
    ...
    return {
        changed: renamedChanged || appendedChanged,
    };
}
