// src/toolingLayer/pageActions/validator/validate/report.ts
...
function renderGroup(group: ValidationRuleGroupResult): void {
    ...
    const groupIcon = iconFor(severity);

    const groupSummary =
        group.warnings === 0 && group.errors === 0
            ? "no issues"
            : `${group.warnings} warning(s), ${group.errors} error(s)`;

    console.log(
        `${groupIcon} ${group.id}  (${group.checks} check${group.checks === 1 ? "" : "s"}, ${colorSummary(groupSummary, severity)})`
    );
...
export function printValidatorHeader(): void {
    printCommandTitle("PAGE ACTION VALIDATOR", "validateIcon");
}

export function printValidatorEnvironment(
    rows: Array<[string, string | number | boolean]>
): void {
    printEnvironment(rows);
}

export function printRuleExecution(
...
export function printValidatorSummary(
    result: ValidationPipelineResult
): void {
    const resultText =
