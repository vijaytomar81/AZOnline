// src/data/data-builder/core/logger.ts
...
export type LoggerOptions = {
  prefix?: string;          // e.g. "[data-builder]"
  verbose?: boolean;        // enables debug()
  withTimestamp?: boolean;  // if true, prepend ISO timestamp
  logToFile?: boolean;      // optional file logging
  logFilePath?: string;     // if logToFile true, default: ./data-builder.log
};
...
export function createLogger(opts: LoggerOptions = {}): Logger {
  const prefix = opts.prefix ?? "[data-builder]";
  const verbose = !!opts.verbose;
  const withTimestamp = opts.withTimestamp !== false; // default TRUE
  const logToFile = !!opts.logToFile;

  const logFilePath =
    opts.logFilePath ?? path.join(process.cwd(), "data-builder.log");
