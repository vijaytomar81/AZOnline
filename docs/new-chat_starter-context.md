...
Transforms Excel test data into structured JSON test cases.
...
- `getCasesFile` loads generated JSON
- `generatedManifest` is the source of truth
...
Executes test cases using JSON + page objects.

## Flow

```
JSON → Case Runner → Step Runner → Page Actions → Assertions → Result
```

## Structure

- core → runners, context, registry
- journeys → business flows
- modes → data / e2e execution
- runtime → step data + scenario loading
...
Provides structured logging across framework.
